...
export default function Home(){
 ...
 return <main className="page">
   ...
   <section className="section"><div className="section-head"><h2>Today at a glance</h2></div><div className="stats-grid">
     <Link to="/deals" className="stat-card stat-link"><strong>{loading?'—':deals.length}</strong><span>Qualifying deals</span></Link><Link to="/deals?quick=New" className="stat-card stat-link"><strong>{loading?'—':newCount}</strong><span>New today</span></Link><Link to="/wardrobe" className="stat-card stat-link"><strong>{loading?'—':items.length}</strong><span>Tracked wardrobe</span></Link><Link to="/deals?quality=Exceptional" className="stat-card stat-link accent"><strong>{loading?'—':exceptional}</strong><span>Exceptional</span></Link>
   </div></section>
